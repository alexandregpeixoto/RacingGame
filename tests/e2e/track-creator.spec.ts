...
test("edits an independent racing line and terrain without breaking the editor", async ({ page }) => {
  await page.goto("/");
  ...
  await page.mouse.click(box.x + 420, box.y + 360);
  await page.mouse.click(box.x + 560, box.y + 360);
  ...
  await expect(page.getByText("Terrain heightmap", { exact: true })).toBeVisible();
});
