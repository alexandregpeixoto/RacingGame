...
describe("track validation extensions", () => {
  it("requires a pit path when pit markers are authored", () => {
    ...
    expect(report.issues.some((item) => item.code === "pit.path-missing")).toBe(
      true,
    );
  });
  ...
  it("accepts a valid racing line and terrain document extension", () => {
    ...
    document.paths.push({
      ...
      widthMeters: 1.4,
      controlPoints: [
        createControlPoint({ x: -30, y: 3 }),
        createControlPoint({ x: 30, y: 3 }),
        createControlPoint({ x: 30, y: 57 }),
        createControlPoint({ x: -30, y: 57 }),
      ],
    });
    const report = validateDocument(document);
    expect(
      report.issues.some((item) => item.code === "path.control-points"),
    ).toBe(false);
    expect(document.terrain?.elevations).toHaveLength(
      (document.terrain?.width ?? 0) * (document.terrain?.height ?? 0),
    );
  });
