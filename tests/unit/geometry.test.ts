...
describe("track geometry", () => {
  it("builds a continuous closed sample loop", () => {
    ...
    expect(path!.positionAtDistance(0).x).toBeCloseTo(
      path!.positionAtDistance(path!.totalLengthMeters).x,
      1,
    );
  });
  ...
  it("reports a valid sample document", () => {
    ...
    expect(
      report.issues.filter((item) => item.severity === "error"),
    ).toHaveLength(0);
  });
  ...
  it("snaps a compatible module to an open connector", () => {
    ...
    const module: TrackModule = {
      id: "new",
      definitionId: "straight",
      transform: { position: { x: -40, y: 0, z: 0 }, rotation: 0 },
      parameters: { length: 40, width: 10, elevationDelta: 0 },
    };
    const connector = getWorldConnector(document.modules[0], "start")!;
    const result = snapModuleToOpenConnector(
      { ...document, modules: document.modules.slice(0, 1), connections: [] },
      module,
      2,
    );
    expect(result.module.transform.position.x).toBeCloseTo(
  ...
  it("samples a closed cubic path independently from the circuit centerline", () => {
    const points = [
      ...
      createControlPoint({ x: 40, y: 0 }),
      createControlPoint({ x: 40, y: 30 }),
      createControlPoint({ x: 0, y: 30 }),
    ];
